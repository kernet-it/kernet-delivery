...
**18.0** branch. Branch-per-version, like OCA: each `NN.0`
...
`18.0.x.y.z` version in `__manifest__.py` and adds that same
...
`18.0.1.0.0` is not an increase. An existing addon creates its
